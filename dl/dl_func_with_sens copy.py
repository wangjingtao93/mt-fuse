...
def trainer(args, sppport_all_task, query_all_task, final_test_task, meta_epoch):
    ...
    for task_idx in sppport_all_task.keys():
        ...
        for epoch in range(args.n_epoch):
            ...
            if len(spec_cls_sens) != 0:
                ...
                if is_val_set_best_sens:
                    ...
                    val_values_all_task_sens[task_idx] = [res_val['loss'], res_val['acc'], res_val['auc'], res_val['prec'], res_val['recall'],
                                                           res_val['f1'], res_val['ka'], res_val['sens'], res_val['spec']] + [best_val_sens,
                                                                                                                            best_val_epoch_sens]
            ...
            with open(str(metric_dir), 'a+') as f:
                ...
                data_row = [task_idx, epoch] + [res_val['loss'], res_val['acc'], res_val['auc'], res_val['prec'], res_val['recall'],
                                                 res_val['f1'], res_val['ka'], res_val['sens'], res_val['spec']]
        ...
        if len(spec_cls_sens) != 0:
            ...
            test_values_all_task_sens[task_idx] = [res_test_sens['loss'], res_test_sens['acc'], res_test_sens['auc'], res_test_sens['prec'],
                                                    res_test_sens['recall'], res_test_sens['f1'],res_test_sens['sens'], res_test_sens['spec']]
            + [best_val_sens, best_val_epoch_sens]
        ...
        with open(str(metric_dir), 'a+') as f:
            csv_write = csv.writer(f, delimiter=',')
            data_row = ['finaltest', ' '] + [res_test['loss'], res_test['acc'],  res_test['auc'], res_test['prec'], res_test['recall'],
                                              res_test['f1'], res_test['ka'], res_test['sens'], res_test['spec']]
            data_row.append(best_val)
            data_row.append(best_val_epoch)
            csv_write.writerow(data_row)
            ...
            spec_cls_sens= []
            if len(spec_cls_sens) != 0:
                data_row = ['finaltest_sens', ' '] + [res_test_sens['loss'], res_test_sens['acc'],  res_test_sens['auc'], res_test_sens['prec'],
                                                       res_test_sens['recall'], res_test_sens['f1'],res_test_sens['ka'], res_test_sens['sens'], 
                                                       res_test_sens['spec']]
                data_row.append(best_val_sens)
                data_row.append(best_val_epoch_sens)
                csv_write.writerow(data_row)

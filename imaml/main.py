...
def run_epoch(epoch, args, model, train_loader, test_ls):
    ...
    meta_test_Q_values, meta_final_acc = dlf.trainer(args, test_ls[0], test_ls[1], test_ls[2], store_dir, epoch)
...
def main(args):
    ...
    # 直接使用
    df_test_s = pd.read_csv(os.path.join(project_path, 'data/linux/all_device/6_2_2_10/four_classes/s_q_t/s.csv'))
    df_test_q = pd.read_csv(os.path.join(project_path, 'data/linux/all_device/6_2_2_10/four_classes/s_q_t/q.csv'))
    df_test_t = pd.read_csv(os.path.join(project_path, 'data/linux/all_device/6_2_2_10/four_classes/s_q_t/t.csv'))
    ...
    for epoch in range(args.n_meta_epoch):
        ...
        res, meta_test_Q_values, meta_final_acc= run_epoch(epoch, args, model, zip(train_support_loader, train_query_loader), test_ls)
